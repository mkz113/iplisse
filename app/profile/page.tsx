...
export default function ProfilePage() {
    ...
    if (loading) return <div className="min-h-screen pt-32 text-center text-slate-400 font-medium animate-pulse">{t.profileLoading}</div>;
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div className="bg-white p-8 md:p-10 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center md:items-start gap-6">
                    ...
                    <div className="text-center md:text-left flex-1">
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">{t.greeting}, {displayName}!</h1>
                ...
                {aiInsights && (
                    <div className="bg-gradient-to-r from-indigo-900 via-blue-900 to-slate-900 p-8 rounded-3xl shadow-xl text-white relative overflow-hidden group">
                        ...
                        <div className="relative z-10 flex items-start gap-5">
                            ...
                            <div>
                                <h3 className="text-blue-300 font-black text-xs uppercase tracking-widest mb-2">{t.aiInsightsTitle}</h3>
                                <p className="text-slate-200 text-sm leading-relaxed">
                                    {t.aiInsightsTextPrefix} <strong className="text-white">{aiInsights.mp} m²</strong> {t.aiInsightsTextMid} <strong className="text-sky-300">{aiInsights.bugs}</strong> {t.aiInsightsTextSuffix} 🛡️🦟
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                {/* Istoric Comenzi */}
                <div>
                    <h2 className="text-xl font-black text-slate-900 mb-6 px-2">{t.orderHistoryTitle}</h2>
                    ...
                    {orders.length === 0 ? (
                        <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center shadow-sm">
                            ...
                            <h3 className="font-bold text-slate-700 mb-2">{t.noCompletedOrders}</h3>
                            <p className="text-slate-500 mb-6 text-sm">{t.noCompletedOrdersDesc}</p>
                                ...
                                <div key={order.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <span className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm ${
                                           ...
                                           {order.status === 'processing' ? t.inWork : order.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="bg-slate-50 rounded-xl p-4 flex justify-between items-center group-hover:bg-blue-50/50 transition-colors">
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t.totalPaidProfile}</span>
